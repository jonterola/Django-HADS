...
from app.forms import QuestionForm, ChoiceForm,UserForm, FilterQuestionForm
...
def index(request):
    ...
    if request.method == "POST":
        form = FilterQuestionForm(request.POST)
        if form.is_valid():
            latest_question_list = Question.objects.filter(category = form.cleaned_data['category']).order_by('-pub_date')
    else:
...
def choice_add(request, question_id):
        ...
        numOpciones = Choice.objects.filter(question=question_id).count()
        numOpcionesCorrectas = Choice.objects.filter(question=question_id, correct=True).count()
        pocasOpciones = ''
        if request.method =='POST':
            form = ChoiceForm(request.POST)
            if form.is_valid():
                choice = form.save(commit = False)
                if numOpcionesCorrectas == 1 and choice.correct:
                    return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form,
                                                        'unicaCorrecta': 'Esta pregunta ya contiene una respuesta correcta.'})
                elif numOpciones == 3 and numOpcionesCorrectas == 0 and not choice.correct :
                    return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ question.question_text,'form': form,
                                                        'unicaCorrecta': 'Las preguntas deben contener UNA respuesta correcta.'})
                elif numOpciones > 3:
                    return render(request, 'polls/choice_new.html', {'muchasOpciones':'Esta pregunta ya contiene 4 opciones. No se pueden añadir más.'})
                else:
                    ...
                    #form.save()
                    if numOpciones < 1:
                        pocasOpciones = 'La pregunta debe contener por lo menos dos opciones.'
